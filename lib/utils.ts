...
// value kuch bhi ho sakta ha ; isliye unknow 
// If this function returns true, then value should be treated as a string.”
export const isHostedUrl = (value: unknown): value is string =>
    typeof value === "string" && value.includes(HOSTING_DOMAIN_SUFFIX);

// crete address aka subdomain aka path for hosting on puter
export const createHostingSlug = () =>
    `roomify-${Date.now().toString(36)}-${Math.random()
        .toString(36)
        .slice(2, 8)}`;

// Ensure domain always ends with: .puter.site
