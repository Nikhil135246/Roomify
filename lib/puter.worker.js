...
router.get('/api/projects/list', async ({ user }) => {
    try {
        ...
        // we saying from list : give everything thing start wih project_prefix and , setting ture for (include values) means give values too of that key 
        const projects = (await userPuter.kv.list( PROJECT_PREFIX ,true)).map(({value}) =>({...value,isPublic:true}));

        // oneliner : get list of key value , isolate only value and add public true tag to each and then we get final projects as an [] array containig only vlues + public ture tag
