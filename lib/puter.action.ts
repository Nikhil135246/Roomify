...
export const createProject = async ({
  ...
  try {
    const response = await puter.workers.exec(
      ...
      { method: "POST", headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({project: payload,visibility}) },
    );
if(!response.ok) {  
  console.error("Failed to save project, response not ok", await response.text());
  return null;
}
